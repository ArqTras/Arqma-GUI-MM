...
if [[ ! -f "${DEVICE_DYLIB}" ]]; then
  DEVICE_DYLIB="${REPO_ROOT}/.prebuilt/arqma-wallet-ffi/$(bash "${REPO_ROOT}/build/ci/resolve-arqma-ffi-release-version.sh")/ios/device/libarqma_wallet_flutter_ffi.dylib"
fi
if [[ ! -f "${DEVICE_DYLIB}" ]]; then
