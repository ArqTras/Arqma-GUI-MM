...
/// Mobile (iOS/Android): remote node only — load GUI `config.json` under [ArqmaPaths.configDir], scan wallet dir like Tauri,
/// start local `arqmad` when daemon type is not `remote`, poll `get_info` for footer sync state.
final class MobileNativeBridge implements NativeBridge {
  MobileNativeBridge();
  ...
  /// Cold reopen: checkpoint had txs/balance — keep syncing until history is refetched.
  bool _walletOpenHistoryPending = false;

  /// Monotonic scan height — ignores one-shot RPC jumps to chain tip.
  ...
  /// Oxen stdout height emit ~1 s during scan; 5 s at tip (remote wallet RPC).
  static const Duration _kMobileWalletHeartbeatIntervalScan =
      Duration(seconds: 1);
  static const Duration _kMobileWalletHeartbeatIntervalTip =
      Duration(seconds: 5);
  ...
  void _syncIosWalletScanLiveActivity({
    ...
    final bool scanning = fullRescan ||
        gap > kWalletDaemonTipToleranceBlocks ||
  ...
  Future<void> _persistConfigSnapshot(
    ...
    try {
      ...
      debugPrint('[MobileNative] persistConfigSnapshot: $e\n$st');
      _showNotification(
  ...
  /// Whether the open wallet file uses a non-empty password (vs empty-password wallets).
  ...
  Future<void> _persistWalletAddressFile(String name, String address) async {
    ...
    try {
      ...
      debugPrint('[MobileNative] write address.txt for "$name": $e\n$st');
    }
  }
  ...
  Future<void> _runConfirmCloseShutdown(ArqmaWalletRpcSession? w) async {
    try {
      ...
      debugPrint('[MobileNative] confirm_close solo pool: $e\n$st');
    }
    try {
      await w?.shutdown().timeout(const Duration(seconds: 3));
    } catch (e, st) {
      debugPrint('[MobileNative] confirm_close shutdown: $e\n$st');
    }
    try {
      exit(0);
    } catch (_) {}
  ...
  Future<dynamic> invoke(String cmd, [Map<String, dynamic>? args]) async {
    ...
    if (cmd == 'app_save_log_level') {
      ...
      if (value.isNotEmpty) {
        try {
          final String dir = (await MobileArqmaPaths.resolve()).configDir;
          ...
          debugPrint('[MobileNative] app_save_log_level: $e');
        }
      }
      return null;
    }
    ...
    if (cmd == 'dialog_open_dir') {
      ...
      try {
        ...
        if (path == null) {
          debugPrint(
              '[MobileNative] dialog_open_dir: user cancelled or picker returned null');
        }
        return path;
      } catch (e, st) {
        debugPrint('[MobileNative] dialog_open_dir failed: $e\n$st');
        return null;
      }
    }
    return null;
  ...
  Future<void> _coreSaveImage(
    ...
    final ArqmaPaths paths = await MobileArqmaPaths.resolve();
  ...
  Future<void> _runCoreStartup() async {
    ...
    final Map<String, dynamic> defaults = buildMobileDefaultsOnly(paths);
    final Map<String, dynamic> ethDefault = Map<String, dynamic>.from(
        buildMobileInitialConfigData(paths)['ethereum'] as Map? ??
            <String, dynamic>{});
    ...
    if (!configFile.existsSync()) {
      final Map<String, dynamic> initial = buildMobileInitialConfigData(paths);
      enforceMobileRemoteOnlyConfig(initial);
    ...
    try {
      ...
      configData = Map<String, dynamic>.from(
          deepMergeMaps(buildMobileInitialConfigData(paths), disk) as Map);
    } catch (e) {
      debugPrint('[MobileNative] config.json parse error: $e');
    ...
    try {
      ...
      debugPrint('[MobileNative] write config.json after startup merge: $e');
  ...
  Future<void> _bestEffortWalletRpcAfterFailure(
    ...
    try {
      ...
      debugPrint('[MobileNative] best-effort tryStart threw: $e\n$st');
      _walletRpc = null;
    }
    if (_walletRpc == null) {
      debugPrint(
          '[MobileNative] best-effort wallet RPC did not start (check daemon address, paths, native FFI .dll/.so/.dylib)');
    }
    _emitWalletBackendState();
  }
  ...
  Duration _mobileWalletHeartbeatInterval() {
    final int dh = _daemonChainTipHeight;
    final int h = _whStoredHeight;
    if (_walletFullRescanUi ||
        _whWalletBackgroundBusy ||
        walletHeightScanningBehind(h, dh)) {
      return _kMobileWalletHeartbeatIntervalScan;
    }
    return _kMobileWalletHeartbeatIntervalTip;
  }

  void _scheduleWalletHeartbeatTick() {
    _walletHeartbeat?.cancel();
    if (_walletRpc == null || _openedWalletDisplayName.isEmpty) {
      return;
    }
    _walletHeartbeat = Timer(_mobileWalletHeartbeatInterval(), () {
      unawaited(
        _walletHeartbeatTick().whenComplete(_scheduleWalletHeartbeatTick),
      );
    });
  }

  /// Oxen/Tauri: push footer height before balance / xfer work finishes.
  void _emitWalletHeightHeartbeatSnapshot({
    required String name,
    required int newH,
  }) {
    if (_walletFullRescanCaughtUp(newH, _daemonChainTipHeight)) {
  ...
  /// Flush wallet + prefs snapshot before the OS suspends the app (keeps `.keys` safe).
  ...
  Future<void> _restoreUiFromCheckpointIfNeeded() async {
    final String name = _openedWalletDisplayName;
    if (name.isEmpty) {
      return;
    }
  ...
  Future<bool> _walletRpcHealthy(ArqmaWalletRpcSession w) async {
    try {
      ...
      if (!walletJsonRpcNoError(gh)) {
        ...
        if (!Platform.isIOS &&
            err.contains('background operation already running')) {
          return true;
        }
        return false;
      }
      ...
      final WalletSessionCheckpoint? cp = await WalletSessionCheckpoint.load();
      return cp != null &&
          cp.walletName == _openedWalletDisplayName &&
          cp.height > 0;
    } catch (e, st) {
  ...
  Future<Map<String, dynamic>?> _callRescanBlockchainWithRetry(
    ArqmaWalletRpcSession w, {
    required bool hard,
  }) async {
    ...
    for (int attempt = 0; attempt < 24; attempt++) {
      try {
        ...
        debugPrint('[MobileNative] rescan_blockchain attempt: $e\n$st');
        last = null;
      }
      if (walletJsonRpcNoError(last)) {
        return last;
      }
      final String msg = walletJsonRpcErrorMessage(last);
      if (!msg.contains('background operation already running')) {
        return last;
      }
  ...
  void _restoreWalletUiAfterRescanFailure() {
    final String name = _openedWalletDisplayName;
    if (name.isEmpty) {
      return;
    }
    final int restoreH = _rescanPreTipHeight > 0
        ? _rescanPreTipHeight
        : _whStoredHeight;
    if (restoreH > 0) {
      _whStoredHeight = restoreH;
    }
    _walletFullRescanUi = false;
    _walletTxListClearedForRescan = false;
    _rescanPreTipHeight = 0;
  ...
  /// Close stray FFI / background jobs before [open_wallet] (account switch, post-sleep).
  ...
  Future<void> _walletXferHeavy(
    ...
    try {
      ...
      if (list.isEmpty && _walletOpenHistoryPending) {
        debugPrint(
            '[MobileNative] skip empty get_transfers emit (open history pending)');
        _whFetchTxPending = true;
        return;
      }
      if (list.isEmpty &&
          ...
          _whLastEmittedTxMaxHeight > 0) {
        debugPrint(
            '[MobileNative] skip empty get_transfers emit (resume transient)');
        return;
      }
      ...
      debugPrint('[MobileNative] wallet get_transfers: $e\n$st');
    } finally {
  ...
  Future<void> _emitWalletAddressBookFromRpc(ArqmaWalletRpcSession w) async {
    try {
      ...
      debugPrint('[MobileNative] set_wallet_address_book: $e\n$st');
    }
  }
  ...
  Future<void> _emitWalletAddressListFromRpc(ArqmaWalletRpcSession w) async {
    try {
      ...
      debugPrint('[MobileNative] set_wallet_address_list: $e\n$st');
    }
  }
  ...
  Future<void> _walletHeartbeatTickBody() async {
    ...
    final int dh = _daemonChainTipHeight;
    final Duration ghCap = const Duration(seconds: 120);
    final Duration abCap = const Duration(seconds: 30);

    Map<String, dynamic>? gh;
    try {
      gh = await w.call('getheight', <String, dynamic>{}).timeout(ghCap);
    } catch (e, st) {
      debugPrint('[MobileNative] wallet hb getheight: $e\n$st');
      gh = null;
    }
    ...
    final bool inScanRhythm = dh == 0 ||
        newH < dh ||
        _whWalletBackgroundBusy ||
        walletHeightScanningBehind(newH, dh);
    if (ghOk) {
      _emitWalletHeightHeartbeatSnapshot(name: name, newH: newH);
    ...
    try {
      ...
      debugPrint('[MobileNative] wallet hb getbalance: $e\n$st');
      gb = null;
    }

    Map<String, dynamic>? ga;
    if (!inScanRhythm || _whAddressBookPending) {
      try {
        ga = await w.call('get_address',
            <String, dynamic>{'account_index': 0}).timeout(abCap);
      } catch (e, st) {
        debugPrint('[MobileNative] wallet hb get_address: $e\n$st');
        ga = null;
      }
    }
  ...
  dynamic _loadRemotes(ArqmaPaths paths) {
    final List<Map<String, dynamic>> nodes = mobileRemoteNodesJson();
    final File f = File(paths.remotesPath);
    try {
      f.parent.createSync(recursive: true);
      f.writeAsStringSync(jsonEncode(nodes));
    } catch (e) {
      debugPrint('[MobileNative] remotes.json write: $e');
    }
    return nodes;
  }
  ...
  Future<void> _emitWalletOpenedUi(String name,
    ...
    if (cpExisting != null && cpExisting.walletName == name) {
      if (bal == 0 && cpExisting.balance > 0) {
        bal = cpExisting.balance;
      }
      if (unl == 0 && cpExisting.unlockedBalance > 0) {
        unl = cpExisting.unlockedBalance;
      }
    }
    ...
    if (isNewWallet) {
      ...
      try {
        ...
        debugPrint('[MobileNative] wallet store after create/restore: $e\n$st');
      }
    }
  ...
  Future<dynamic> _walletStubBackendSend(String module, String method,
              ...
              if (addr.isNotEmpty) {
                ...
                unawaited((() async {
                  try {
                    ...
                    debugPrint(
                        '[MobileNative] add_address_book after transfer: $e\n$st');
                  }
                })());
              }
            }
    ...
    if (method == 'rescan_blockchain') {
      ...
      if (w != null) {
        ...
        final Map<String, dynamic>? res =
            await _callRescanBlockchainWithRetry(w, hard: hard);
        if (!walletJsonRpcNoError(res)) {
          final String msg = walletJsonRpcErrorMessage(res,
    ...
    if (method == 'rescan_spent') {
      ...
      if (w != null) {
        try {
          ...
          debugPrint('[MobileNative] rescan_spent: $e\n$st');
        }
        unawaited(_refreshWalletUiAfterRescan(clearTransactions: false));
      }
      return <String, dynamic>{};
    ...
    if (method == 'export_transactions') {
      ...
      try {
        ...
        debugPrint('[MobileNative] export_transactions: $e');
        _emit(<String, dynamic>{
          'event': 'set_tx_status',
          'data': <String, dynamic>{
            'code': -99,
    ...
    debugPrint('[MobileNative] unhandled wallet::$method');
    return <String, dynamic>{};
  }
  ...
  void _removeWalletFilesOnDisk(String wdir, String walletName) {
    final String sep = Platform.pathSeparator;
    for (final String suffix in <String>['', '.keys', '.address.txt', '.meta.json']) {
      try {
        File('$wdir$sep$walletName$suffix').deleteSync();
      } catch (_) {}
    }
  }
  ...
  Future<void> _emitWalletListAfterDelete(Map<String, dynamic> cfg) async {
    final String? wdir = walletFilesDir(cfg);
    if (wdir != null) {
      _emit(<String, dynamic>{
        'event': 'wallet_list',
