...
/// Footer / scan banner / tx list daemon tip — skip notify when only peer extras change.
...
class GatewayStore extends ChangeNotifier {
  ...
  void setWalletInfo(Map<String, dynamic> data) {
    ...
    // Mobile wallet heartbeat emits `scan_poll_ts` every 5 s even at tip — skip
    // full-tree rebuilds when nothing visible changed.
    if (uiChanged) {
      _notify();
    }
  }
