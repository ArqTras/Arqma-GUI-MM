...
- CI / native upstream: default **`ARQMA_UPSTREAM_REPO`** is **`https://github.com/arqtras/arqma.git`** (fork); **`ARQMA_UPSTREAM_REF`** defaults to **`pospow`** (`build/ci/clone-arqma.sh`). Official **`arqmad`** binaries still from **`arqma/arqma` Releases**.
