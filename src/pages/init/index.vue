...
export default defineComponent({
  ...
  setup () {
    ...
    // Computed props
    const status = computed(() => $store.state.gateway.app.status)
    const remote = computed(() => $store.state.gateway.app.selected_node)
    const app = computed(() => $store.state.gateway.app)
    const daemons = computed(() => $store.state.gateway.daemons)
    ...
    const updateStatus = async () => {
      try {
        switch (status.value.code) {
          ...
          case 5:
            $q.notify({
              type: "warning",
              timeout: 2000,
              message: t("pages.init.daemon_not_found")
            })
            break
          case 6:
        ...
        await api.error("pages/init/index", "updateStatus", error.stack || error)
      }
    }
