...
final class MobileNativeBridge implements NativeBridge {
  ...
  /// In-memory only — used to reopen the wallet after suspend stranded the FFI session.
  ...
  Future<void> recoverWalletSessionAfterForeground() {
    return _runWalletSyncLane(() async {
      ...
      if (_walletSessionStale && !_walletFullRescanUi) {
        try {
          await w
              .call('refresh', <String, dynamic>{})
              .timeout(const Duration(seconds: 45), onTimeout: () => null);
  ...
  Future<void> _walletHeartbeatTickBody() async {
    ...
    final int dh = _daemonChainTipHeight;
    const Duration ghCap = Duration(seconds: 120);
    const Duration abCap = Duration(seconds: 30);
