...
class _DaemonSettingsDialogState extends State<_DaemonSettingsDialog> {
  int _tab = 0;
  final GlobalKey<SettingsGeneralPanelState> _panelKey = GlobalKey<SettingsGeneralPanelState>();
  ...
  static bool _daemonIsRemote(GatewayStore store) {
    ...
    final Map<String, dynamic> app = Map<String, dynamic>.from(cfg['app'] as Map? ?? <String, dynamic>{});
    final String net = '${app['net_type'] ?? 'mainnet'}';
    final Map<String, dynamic> daemons = Map<String, dynamic>.from(cfg['daemons'] as Map? ?? <String, dynamic>{});
    final Map<String, dynamic> d = Map<String, dynamic>.from(daemons[net] as Map? ?? <String, dynamic>{});
  ...
  Future<void> _onPeerTap(BuildContext context, Map<String, dynamic> entry) async {
    ...
    final bool? wantBan = await showDialog<bool>(
      ...
      builder: (BuildContext c) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(c, false), child: Text(loc.tr('components.settings.peer_details_cancel_label'))),
          TextButton(onPressed: () => Navigator.pop(c, true), child: Text(loc.tr('components.settings.peer_details_ok_label'))),
        ],
      ),
    );
    ...
    final int? picked = await showDialog<int>(
      ...
      builder: (BuildContext c) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(c), child: Text(loc.tr('components.settings.peer_details_cancel_label'))),
          TextButton(
            onPressed: () => Navigator.pop(c, int.tryParse(seconds.text.trim()) ?? 3600),
    ...
    await api.send('daemon', 'ban_peer', <String, dynamic>{'host': host, 'seconds': picked});
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<dynamic> connections = (store.daemon['connections'] as List<dynamic>?) ?? const <dynamic>[];
    final List<dynamic> bans = (store.daemon['bans'] as List<dynamic>?) ?? const <dynamic>[];

    return Dialog(
      backgroundColor: const Color(0xFF1a1a1a),
      insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
      child: ConstrainedBox(
        constraints: BoxConstraints(maxWidth: 720, maxHeight: MediaQuery.of(context).size.height * 0.88),
        child: Column(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(child: Text(loc.tr('components.settings.settings'), style: Theme.of(context).textTheme.titleLarge)),
                  if (!remoteOnly)
                    ToggleButtons(
                  ...
                  TextButton(onPressed: () => _saveAndClose(context), child: Text(loc.tr('components.settings.save'))),
                ],
              ),
            ),
            ...
            Expanded(
              child: _tab == 0 || remoteOnly
                  ...
                  : ListView(
                      ...
                      children: [
                        Text(loc.tr('components.settings.peer_list'), style: Theme.of(context).textTheme.titleSmall),
                        if (connections.isEmpty)
                          Padding(
                            padding: const EdgeInsets.only(top: 8),
                            child: Text(loc.tr('components.settings.no_daemon_connections')),
                          )
                        else
                          ...connections.map((dynamic e) {
                            final Map<String, dynamic> m = Map<String, dynamic>.from(e as Map);
                            final String addr = '${m['address'] ?? m['ip'] ?? m['host'] ?? ''}';
                        ...
                        if (bans.isNotEmpty) ...[
                          ...
                          Text(loc.tr('components.settings.banned_peers'), style: Theme.of(context).textTheme.titleSmall),
                          ...bans.map((dynamic e) {
                            final Map<String, dynamic> m = Map<String, dynamic>.from(e as Map);
                            final String host = '${m['host'] ?? ''}';
                            final int sec = (m['seconds'] as num?)?.toInt() ?? 0;
                            final String until = DateTime.now().add(Duration(seconds: sec)).toLocal().toString();
                            return ListTile(
                              dense: true,
                              title: Text(host, style: const TextStyle(fontSize: 12)),
                              subtitle: Text('${loc.tr('components.settings.banned_until')} $until'),
                            );
                          }),
                        ],
                      ],
