...
/// Deferred Touch ID enrollment after wallet open (macOS needs a stable window).
class PendingBiometricEnable {
  const PendingBiometricEnable({
...
class WalletBiometricUnlock {
  ...
  static void scheduleEnable(PendingBiometricEnable pending) {
    _pendingEnable = pending;
    debugPrint(
        '[WalletBiometricUnlock] scheduled Touch ID enable for ${pending.walletName}');
  }

  static Future<void> flushPendingEnable() async {
    final PendingBiometricEnable? pending = _pendingEnable;
    if (pending == null) {
      return;
    }
    _pendingEnable = null;
    ...
    debugPrint('[WalletBiometricUnlock] flushing pending Touch ID enable');
  ...
  static Future<void> enable({
    ...
    if (!await isPlatformSupported()) {
      throw StateError('Touch ID is unavailable on this Mac');
    }
    if (password.isEmpty) {
      throw StateError('Wallet password is required to enable Touch ID');
    }
    final bool ok = await _authenticate(localizedReason: localizedReason);
    if (!ok) {
      throw StateError('Touch ID confirmation was cancelled');
    }
