...
**CI note:** **[desktop release CI](../../.github/workflows/desktop-release.yml)** — **`flutter-linux`** and **`flutter-windows`** use the **same** upstream for **`wallet_merged`**: fork **[arqtras/arqma](https://github.com/arqtras/arqma)** branch **`pospow`** (`ARQMA_UPSTREAM_REPO` / `ARQMA_UPSTREAM_REF` at workflow level). The **`arqmad`** binary is **not** built from that tree on Linux; both platforms take **`arqmad`** from the **[latest GitHub Release of arqma/arqma](https://github.com/arqma/arqma/releases/latest)** (`build/download-binaries.js`): Linux runs **`build/ci/fetch-arqmad-github-release.sh`** (extract → `./bin` → **`build/copy-to-tauri-bins.js`** → `rust/tauri-app/src-tauri/bin/`), Windows uses **`build/ci/flutter-windows-fetch-arqma-binaries.ps1`**. Local Tauri builds can still use **`build/ci/build-arqma-daemon-copy.sh`** to compile **`daemon`** from a CMake tree when needed.
