...
**CI note:** **[desktop release CI](../../.github/workflows/desktop-release.yml)** — **`flutter-linux`**, **`flutter-macos`**, and **`flutter-windows`** download prebuilt **`arqma-wallet-flutter-ffi`** from **[ArqTras/FFI Latest](https://github.com/ArqTras/FFI/releases/latest)** via **`build/ci/ensure-desktop-latest-ffi.sh`** + **`fetch-arqma-desktop-prebuilts.*`** (desktop GUI always refreshes Latest; pin only with **`ARQMA_FFI_DESKTOP_ALLOW_PIN=1`**). **`arqmad`** comes from the latest **[arqma/arqma](https://github.com/arqma/arqma/releases/latest)** release. Local dev: **`flutter/arqma_wallet_gui/tool/fetch_latest_wallet_ffi.sh`** (or build FFI from source via **`rust/tool/build_wallet_flutter_ffi.sh`**).
