...
**CI note:** **[desktop release CI](../../.github/workflows/desktop-release.yml)** — **`flutter-linux`**, **`flutter-macos`**, and **`flutter-windows`** download prebuilt **`arqma-wallet-flutter-ffi`** from **[ArqTras/FFI](https://github.com/ArqTras/FFI/releases/tag/1.0.1)** (`build/ci/fetch-arqma-wallet-ffi-release-linux.sh` / `.ps1`, tag **`ARQMA_FFI_RELEASE_VERSION`**, default **`1.0.1`**: `linux-x86_64`, `macos-arm64`, `windows-x86_64-gnu`). **`arqmad`** comes from the latest **[arqma/arqma](https://github.com/arqma/arqma/releases/latest)** release (Linux/macOS: **`fetch-arqmad-github-release.sh`**, Windows: **`flutter-windows-fetch-arqma-binaries.ps1`**). Local dev can still build FFI from source via **`build/ci/build-arqma-*.sh`** and **`cargo build -p arqma-wallet-flutter-ffi`**.
