...
**Linux / macOS (portable bundles, AppImage, tarballs):** the default `cdylib` link leaves **`NEEDED`** entries for Boost/OpenSSL/etc. matching the **build host** (e.g. Ubuntu’s `libboost_program_options.so.1.83.0`). Other machines then fail `dlopen` with “no such file” even though `libarqma_wallet_flutter_ffi.so` is present. **Experimental:** set **`ARQMA_WALLET_FFI_STATIC_HYBRID=1`** when running `cargo build -p arqma-wallet-flutter-ffi` to fold some deps in statically (ICU + `libstdc++` stay dynamic — see `build.rs`). **CI release builds do not set this:** distro static archives (e.g. `libzmq.a` / `libunbound.a` on Ubuntu) are often **not** `-fPIC`, so linking them into a shared library fails with relocation errors; the macOS hybrid path also mixes GNU `ld` flags with Apple link rules. Use dynamic linking in **`.github/workflows/desktop-release.yml`** unless you rebuild those deps as PIC / adjust `build.rs` for a portable static-hybrid link.
