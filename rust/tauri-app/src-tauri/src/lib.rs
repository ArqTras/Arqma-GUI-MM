...
async fn wallet_exit_shutdown_handle(st: &AppData) {
    if st.wallet_exit_shutdown_done.swap(true, Ordering::SeqCst) {
        // #region agent log
...
fn app_save_log_level(app: tauri::AppHandle, value: String) -> Result<(), String> {
    ...
    if p.exists() {
        let _ = fs::File::open(&p).and_then(|mut f| f.read_to_string(&mut s));
    }
...
fn dialog_open_dir(default_path: String) -> Option<String> {
    ...
    b.pick_folder().map(|p| p.to_string_lossy().to_string())
}
        ...
        "wallet" => {
            ...
            "[wallet] get_coin_price: exceeded {PRICE_AGGREGATE_SECS}s aggregate HTTP — emitting zeros"
                ...
                if message.method == "close_wallet" {
                    ...
                    // #endregion
                    {
                        ...
                        if let Some(mut h) = b.wh_xfer_task.take() {
                            ...
                            let xfer_outcome =
                                tokio::time::timeout(Duration::from_secs(xfer_cap_secs), &mut h)
                                    .await;
              ...
              "close_wallet: wallet_rpc_lane acquire timed out after {lane_deadline_secs}s",
                ...
                let wallet_res = {
                    let mut b = state.backend.lock().await;
                    crate::wallet_handler::handle_wallet(&app, &mut b, http, &message.method, data)
                        .await
                };
                let wallet_res = if message.method == "open_wallet" {
                    match wallet_res {
                        Ok(v) => Ok(v),
                        Err(e)
                            if e.contains("open_wallet RPC timed out")
                                || e.contains("open_wallet RPC failed") =>
                        {
...
fn try_create_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let _ = TrayIconBuilder::new()
        ...
        .on_tray_icon_event(move |_, e| match e {
            TrayIconEvent::Click {
