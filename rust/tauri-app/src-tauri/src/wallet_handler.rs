...
pub async fn handle_wallet(
    ...
    match method {
        ...
        "has_password" => {
            ...
            let stored = st.wallet_password_hash_hex.as_deref().unwrap_or("");
        ...
        "copy_old_gui_wallets" => {
            ...
            let list = p
                ...
                .unwrap_or(&[]);
            let failed =
                crate::wallet_copy_old_gui::run_copy_old_gui_wallets(&st.config_data, list)?;
            BackendReceiveSink::emit_receive(
                app,
                "set_old_gui_import_status",
        ...
        "remove_signature_data" => {
            eprintln!(
                "[wallet] remove_signature_data: no ZMQ channel (same as Node — no-op in Tauri)"
            );
        }
        "cancel_stake" => {}
        "relay_sweepAll" => {
        ...
        "validate_address" | "open_wallet" | "close_wallet" | "create_wallet" | "save_wallet"
        | "transfer" | "rescan_blockchain" | "rescan_spent" => {
                    ...
                    if method == "open_wallet" {
                        ...
                        .await
                        {
                            ...
                            Err(_) => {
                                ...
                                return Err(format!(
                                    "open_wallet RPC timed out after {open_timeout}s"
                                ));
                            }
                        }
                        ...
                        .await
                        {
                            Ok(Ok(v)) => v,
                            Ok(Err(e)) => {
                                return Err(format!("save_wallet: store RPC failed: {e}"))
                            }
                            Err(_) => {
                                return Err(format!(
                ...
                if let Some(res) = r.get("result") {
                        ...
                        == Some(true)
                    {
                        if let Some(addr) = p.get("address").and_then(|a| a.as_str()) {
                            if !addr.is_empty() {
                                let ab =
                                    p.get("address_book").cloned().unwrap_or_else(|| json!({}));
                                let add_p = json!({
                                  "address": addr,
            ...
            if method == "open_wallet" {
                if !st.wallet_salt.is_empty() {
                    if let Some(pass) = p.get("password").and_then(|x| x.as_str()) {
                        if let Ok(h) =
                            crate::wallet_password::pbkdf2_password_hex(pass, &st.wallet_salt)
                        {
              ...
              "[wallet] open_wallet: getheight snapshot timed out after {snap_t}s (heartbeat will refresh)"
                    ...
                    if !addr_path.exists() {
                        ...
                        .await
                        {
                            Ok(Ok(a)) if crate::json_util::json_rpc_no_error(&a) => {
                                if let Some(addr) =
                                    a.pointer("/result/address").and_then(|x| x.as_str())
                                {
                ...
                match tokio::time::timeout(Duration::from_secs(25), w.call("query_key", &p_sp))
                    .await
                {
...
async fn wallet_restore_view_wallet(
    ...
    BackendReceiveSink::emit_receive(app, "reset_wallet_error", json!({}))?;
    let mut refresh_h =
        match crate::wallet_rpc_electron::resolve_restore_refresh_height(st, http, p).await {
            Ok(h) => h,
            Err(e) => {
                BackendReceiveSink::emit_receive(
                    app,
...
async fn wallet_import_wallet(
    ...
    BackendReceiveSink::emit_receive(app, "reset_wallet_error", json!({}))?;
    let filename =
        wallet_filename_from_params(p).ok_or_else(|| "import_wallet: name".to_string())?;
...
async fn wallet_delete_wallet(
    ...
    BackendReceiveSink::emit_receive(app, "show_loading", json!({ "message": "Deleting wallet" }))?;
            ...
            if let Some(o) = transaction.as_object_mut() {
                ...
                if let Some(dest) = o.get("destinations").and_then(|d| d.as_array()) {
                    if !dest.is_empty() {
                        o.insert(
                            "destinations".into(),
                            json!(serde_json::to_string(dest).unwrap_or_default()),
                        );
                    }
                }
...
fn emit_validate_address_from_rpc_result(
    ...
    let Some(res) = r.get("result") else {
        BackendReceiveSink::emit_receive(
            app,
            "set_valid_address",
            json!({ "address": address, "valid": false }),
        )?;
        return Ok(());
    };
...
fn map_wallet_rpc(method: &str, p: &Value) -> Result<(String, Value), String> {
    ...
    match method {
        "validate_address" => Ok(v(
            "validate_address",
            json!({ "address": p.get("address") }),
        )),
        "open_wallet" => {
            let filename = p
                .get("name")
