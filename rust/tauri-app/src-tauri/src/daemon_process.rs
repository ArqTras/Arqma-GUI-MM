...
pub async fn ensure_daemon_for_startup(
    ...
    let p2p_ip = d
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("0.0.0.0");
    let p2p_p = d.get("p2p_bind_port").and_then(port_u64).unwrap_or(19_993);
    let rpc_ip = d
        .get("rpc_bind_ip")
        .and_then(|v| v.as_str())
        .unwrap_or("127.0.0.1");
    let rpc_p = d.get("rpc_bind_port").and_then(port_u64).unwrap_or(19_994);
    let out_p = d.get("out_peers").and_then(num_str).unwrap_or("-1".into());
    let in_p = d.get("in_peers").and_then(num_str).unwrap_or("-1".into());
    let lim_up = d
        .get("limit_rate_up")
        .and_then(num_str)
        .unwrap_or("-1".into());
                ...
                if let Some(dproc) = st.daemon_process.as_mut() {
                    match dproc.try_wait() {
                        Ok(Some(status)) if !status.success() => {
                            ...
                            return Err(
                                "arqmad process exited before get_info (check logs)".to_string()
                            );
                        }
                        _ => {}
                    }
