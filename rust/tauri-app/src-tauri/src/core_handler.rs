...
pub async fn handle_core(
    ...
    match method {
        ...
        "change_remotes" => {
            ...
            sink.emit_receive("set_app_data", json!({ "remotes": st.remotes }))?;
        }
        "change_ethereum" => {
            let e = st
                .config_data
            ...
            st.config_data = merge_json(&st.config_data, &json!({ "ethereum": merged }));
            st.ethereum = st
                .config_data
                .get("ethereum")
                .cloned()
        ...
        "change_scan" => {
            st.config_data = merge_json(&st.config_data, &json!({ "app": { "scan": params } }));
        ...
        "quick_save_config" => {
            ...
            st.ethereum = st.config_data.get("ethereum").cloned().unwrap();
        ...
        "save_config_init" => {
            ...
            write_config_file(&st.paths, &st.config_data).map_err(|e| e.to_string())?;
            st.shutdown_subprocesses_async(http, _rpc_lane_shutdown)
                .await;
        ...
        "save_pool_config" => {
            ...
            let old_enabled = st
                ...
                .unwrap_or(false);
            let merged_pool = merge_json(st.config_data.get("pool").unwrap_or(&json!({})), params);
            let normalized_bind_ip = merged_pool
                .get("server")
                .and_then(|s| s.get("bindIP"))
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let merged_pool = if normalized_bind_ip.is_empty()
                || normalized_bind_ip == "0.0.0.0"
                || normalized_bind_ip == "127.0.0.1"
            {
                merge_json(
                    &merged_pool,
...
fn normalize_pool_var_diff(pool: Value) -> Value {
    let vd = pool.get("varDiff").cloned().unwrap_or_else(|| json!({}));
    let start = vd
        .get("startDiff")
        .and_then(|v| v.as_u64())
        .unwrap_or(50_000)
