...
use crate::json_rpc_client::{daemon_post, WalletRpcClient};
...
pub async fn run_pool_tick(app: &AppHandle, http: &Client, w: &WalletRpcClient, config: &Value) {
    ...
    for pool in states {
        let total_c = pool
            ...
            .and_then(|x| x.as_f64())
            .or_else(|| {
                pool.get("total_contributed")
                    .and_then(|x| x.as_u64().map(|u| u as f64))
            })
            .unwrap_or(0.0);
                ...
                if cont.iter().any(|k| {
                    ...
                    let amount: f64 = cont
                        ...
                        .filter_map(|c| {
                            ...
                            {
                                c.get("amount")
                                    .and_then(|a| a.as_f64().or(a.as_u64().map(|u| u as f64)))
                            } else {
                                None
                            }
                        })
                        .sum();
                    let eq = if total_c > 0.0 {
                        (amount / total_c) * 100.0
                    } else {
                        0.0
                    };
            ...
            let amount: f64 = pool
                ...
                .map(|a| {
                    a.iter()
                        ...
                        .filter_map(|c| {
                            c.get("amount")
                                .and_then(|m| m.as_f64().or(m.as_u64().map(|u| u as f64)))
                        })
                        .sum()
                })
                .unwrap_or(0.0);
    ...
    st.stake_acquisition_task = Some(tokio::spawn(async move {
        ...
        while app.try_state::<AppData>().is_some() {
            ...
            let (cfg, fork) = {
                ...
                (
                    b.config_data.clone(),
                    b.wallet.as_ref().unwrap().fork_for_heartbeat(),
                )
            };
