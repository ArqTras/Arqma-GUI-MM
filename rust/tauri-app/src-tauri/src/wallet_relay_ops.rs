...
pub fn cancel_transaction(st: &mut WalletBackendState, p: &Value) {
    ...
    st.tx_metadata_list.retain(|m| m.kind != t);
}
...
pub async fn relay_sweep_all(
    ...
    let items: Vec<WalletTxMetadata> = st
        ...
        .collect();
    for t in &items {
        let r = w.call("relay_tx", &json!({ "hex": t.tx_metadata })).await;
        match r {
    ...
    st.tx_metadata_list.retain(|m| m.kind != "sweepAll");
}

pub async fn relay_transfer(app: &AppHandle, st: &mut WalletBackendState, w: &WalletRpcClient) {
    let mut err = String::new();
    let items: Vec<WalletTxMetadata> = st
        .tx_metadata_list
        .iter()
    ...
    st.tx_metadata_list.retain(|m| m.kind != "transfer_split");
}
...
pub fn push_stake_metadata(st: &mut WalletBackendState, p: &Value, r: &Value) {
    st.tx_metadata_list.retain(|m| m.kind != "stake");
    let Some(res) = r.get("result") else { return };
    let Some(h) = res.get("tx_metadata") else {
        return;
    };
    let hex = h
        .as_str()
