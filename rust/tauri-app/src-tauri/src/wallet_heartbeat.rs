    ...
    while app.try_state::<AppData>().is_some() {
        ...
        let sleep_ms = {
            ...
            if pressure == 0 {
                ...
                base.max(420u64.saturating_mul(1 + u64::from(pressure.min(28))))
                    .min(12000)
            }
        };
        if is_sync_debug() {
            ...
            let (wh, dh, pr) = {
                ...
                (
                    b.wh_stored_height,
                    b.daemon_last_height,
                    b.wh_light_rpc_pressure,
                )
            };
            eprintln!(
...
async fn tick_once(app: &AppHandle, c: &WalletRpcClient) -> bool {
    ...
    let (
        ...
        let in_scan_rhythm = dh == 0 || b.wh_stored_height < dh;
    ...
    let _lane_gh = match rpc_lane.clone().acquire_owned().await {
        ...
        Err(e) => {
            eprintln!(
                "[wallet hb] tick: wallet_rpc_lane acquire(getheight): {}",
                e
            );
            return false;
        }
    ...
    let xfer_trigger = force_fetch_tx || has_balance_change || (!in_scan_rhythm && new_h != h0);
    ...
    if xfer_trigger {
        if matches!(&gb, Ok(v) if json_rpc_no_error(v)) {
            ...
            if let Ok(wallet_lane_hold) = xfer_lane_heavy_bg.clone().try_acquire_owned() {
                ...
                let opt_ga = if let Ok(v) = &ga {
                    Some(v.clone())
                } else {
                    None
                };
                let opt_gb = if let Ok(v) = &gb {
                    Some(v.clone())
                } else {
                    None
                };
            ...
            for ps in parr {
                if index_u64(ps.get("address_index")) == Some(need) {
                    if let Some(m) = a.as_object_mut() {
                        m.insert(
                            "balance".into(),
                            ps.get("balance").cloned().unwrap_or(json!(null)),
                        );
                        m.insert(
                            "unlocked_balance".into(),
                            ps.get("unlocked_balance")
                                .cloned()
...
async fn top_up_unused_subaddresses(c: &WalletRpcClient, al: Value) -> Option<Value> {
    ...
    while unused.len() < LIMIT {
        let r = c
            .call("create_address", &json!({ "account_index": 0 }))
            .await
            .ok()?;
...
pub async fn fetch_address_book_map(c: &WalletRpcClient) -> Result<Value, String> {
    ...
    for e in entries {
        ...
        let is_star = e.get("starred").and_then(|s| s.as_bool()).unwrap_or(false);
        if is_star {
            starred.push(e);
        } else {
            book.push(e);
