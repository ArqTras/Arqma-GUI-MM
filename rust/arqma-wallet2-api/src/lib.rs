...
// Whole-archive + bundle: required on macOS (and Linux cdylib) so unreferenced `.o` from static
// archives are not dropped. On `x86_64-pc-windows-gnu`, rustc rejects `#[link(..., modifiers)]`
// when `build.rs` also passes `-Wl,--whole-archive` (conflicting link modifiers). MinGW: plain
// `static` here; whole-archive for RandomX stays in `build.rs`.
#[cfg(all(
