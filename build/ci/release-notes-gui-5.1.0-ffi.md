...
Desktop and mobile bundles built from tag **5.1.0** with prebuilt wallet FFI from the **Latest** [ArqTras/FFI](https://github.com/ArqTras/FFI/releases/latest) release (currently **1.0.4+**).

### Native wallet (FFI + GUI)

- **FFI:** Desktop and mobile use the same Latest ArqTras/FFI tag (`resolve-arqma-ffi-release-version.*`). Pin with `ARQMA_FFI_RELEASE_VERSION=1.0.4` if needed.
...
- **Desktop release (Flutter):** `fetch-arqma-wallet-ffi-release*` + `fetch-arqma-wallet-solo-pool-release*` (Latest FFI).
