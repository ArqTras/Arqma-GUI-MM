...
# Native `wallet_merged` must still be built from **arqma/arqma** via `build/ci/build-arqma-*.sh` — this script only supplies the daemon executable for `rust/tauri-app/src-tauri/bin/`.
set -eu
