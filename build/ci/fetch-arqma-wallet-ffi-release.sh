...
export ARQMA_FFI_RELEASE_VERSION="${ARQMA_FFI_RELEASE_VERSION:-1.0.5}"
case "$(uname -s 2>/dev/null || echo unknown)" in
  Linux|Darwin)
    export ARQMA_FFI_PLATFORMS="${PLATFORMS}"
    exec bash "${ROOT}/build/ci/fetch-arqma-wallet-ffi-release-linux.sh"
    ;;
esac
if command -v pwsh >/dev/null 2>&1; then
  ...
  echo "fetch-arqma-wallet-ffi-release.sh needs PowerShell on Windows or bash fetch on Linux/macOS" >&2
  exit 1
fi
